...
/**
 * 云鱼商城后端分页结构的 transform
 *
 * 后端 `PageResult<T>` 为 `{ list, total, page, pageSize }`，与 soybean 默认的 `{ records, current, size, total }` 不一致，这里单独适配。
 */
export function mallTransform<ApiData>(
