...
> GitHub / Gitee 的 Markdown 渲染**不会自动生成目录导航**，故此处手写一份，标题锚点可点击直达。

- [在线演示（视频）](#在线演示视频)
- [0. 项目一句话](#0-项目一句话)
- [1. 功能模块图（第一屏 · 先看清能力）](#1-功能模块图第一屏--先看清能力)
- [2. 技术架构（第二屏 · 讲清怎么选型）](#2-技术架构第二屏--讲清怎么选型)
- [3. 快速开始（详细使用）](#3-快速开始详细使用)
- [4. 功能实现详解（第三屏 · 核心逻辑讲透）](#4-功能实现详解第三屏--核心逻辑讲透)
- [5. 踩坑经验总结（重点 · 全是真实教训）](#5-踩坑经验总结重点--全是真实教训)
- [6. 后端资产清单](#6-后端资产清单)
- [7. 部署上线流程](#7-部署上线流程)
- [8. 开源协作约定](#8-开源协作约定)
- [联系作者（微信同号）](#联系作者微信同号)
...
面向 C 端消费者的**生鲜电商小程序**，覆盖「浏览 → 加购 → 下单 → **微信支付（真实到账）** → 发货 → 收货 → 售后退款（支持单件指定商品部分退款）」完整闭环，配一套功能齐全的商家管理后台。后端为**单体架构**，无网关、无 Nacos、不引入多余分布件复杂度；并发扣库存 / 支付幂等由 **Redis + Redisson** 在业务层解决。
...
flowchart LR
    ...
    DONE -->|7天内申请| RF[售后退款 仅退款不退货<br/>支持单件部分退款<br/>微信原路退回]
```
...
- **支持部分退款吗？** 支持。可按订单内**指定的某一件商品（一笔 order_item）**发起「仅退款、不退货」：退款金额为该商品小计、并受订单剩余可退金额上限约束（`0 < 退款额 ≤ 剩余可退`）；退款成功按该 item 回滚库存。生鲜不退货物，故不支持「退货+退款」式整单退货；全额退款同样支持。